...
class WebSocket:
    ...
    def recv(self):
        ...
        if dwError == ERROR_SUCCESS:
            if eBufferType.value in [WINHTTP_WEB_SOCKET_UTF8_MESSAGE_BUFFER_TYPE,
                                     WINHTTP_WEB_SOCKET_UTF8_FRAGMENT_BUFFER_TYPE]:
                return pbCurrentBufferPointer[:dwBytesTransferred.value].decode('utf8')
            elif eBufferType.value in [WINHTTP_WEB_SOCKET_BINARY_MESSAGE_BUFFER_TYPE,
    ...
    def connect(self, url, header=None, http_proxy_host=None, http_proxy_port=None):
        ...
        self.hSession = AutoWinHttpHandle(
            WinHttpOpen("WebSocket Client", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME,
        ...
        hRequest = AutoWinHttpHandle(
            WinHttpOpenRequest(self.hConnect, "GET", path, None, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
                               flag))
