...
class TS(basetrans):
    ...
    def checkpath(self):
        ...
        self.config['用户词典1(可选)'], self.config['用户词典2(可选)'], self.config['用户词典3(可选)']):
            ...
            self.engine = autoproc(subproc_w(
                './files/plugins/shareddllproxy32.exe jbj7 "{}" {} {} '.format(self.dllpath, pipename,
                                                                               waitsignal) + dictpath, name='jbj7'))
    ...
    def packuint32(self, i):  # int -> str
        return bytes(chr((i >> 24) & 0xff) + chr((i >> 16) & 0xff) + chr((i >> 8) & 0xff) + chr(i & 0xff),
                     encoding='latin-1')
