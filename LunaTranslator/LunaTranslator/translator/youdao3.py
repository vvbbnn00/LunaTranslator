...
class TS(basetrans):
    ...
    def translate(self, content):
        ...
        response = self.session.post('https://m.youdao.com/translate', data=data, headers=headers,
                                     cookies={'_yd_btn_fanyi_29': 'true',
                                              '_yd_newbanner_day': '29', }).text

        return \
        re.search('<ul id="translateResult">([\\s\\S]*?)<li>([\\s\\S]*?)</li>([\\s\\S]*?)<\/ul>', response).groups()[1]
