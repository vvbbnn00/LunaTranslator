...
class QUnFrameWindow(resizableframeless):
    ...
    def showline(self, clear, res, color, type_=1, origin=True):
        ...
        if globalconfig['zitiyangshi'] == 2:
            self.translate_text.mergeCurrentCharFormat_out(globalconfig['miaobiancolor'], color,
                                                           globalconfig['miaobianwidth2'])
        elif globalconfig['zitiyangshi'] == 4:
            self.translate_text.mergeCurrentCharFormat_out(color, globalconfig['miaobiancolor'],
                                                           globalconfig['miaobianwidth2'])
    ...
    def refreshtoolicon(self):
        ...
        iconstate = {'fullscreen': self.isletgamefullscreened, "muteprocess": self.processismuteed, "locktoolsbutton":
            globalconfig['locktools'], "showraw": globalconfig['isshowrawtext'],
                     "automodebutton": globalconfig['autorun']}
        colorstate = {"automodebutton": globalconfig['autorun'], "showraw": globalconfig['isshowrawtext'],
                      "mousetransbutton": self.mousetransparent, "backtransbutton": self.backtransparent,
                      "locktoolsbutton": globalconfig['locktools'], "hideocrrange": self.showhidestate,
                      "bindwindow": self.isbindedwindow, "keepontop": globalconfig['keepontop']}
        onstatecolor = "#FF69B4"
        ...
        for i in range(len(self.buttons)):
            ...
            if name in iconstate:
                icon = globalconfig['toolbutton']['buttons'][name]['icon'] if iconstate[name] else \
                globalconfig['toolbutton']['buttons'][name]['icon2']
            else:
    ...
    def addbuttons(self):
        ...
        functions = (
            ...
            ("resize", lambda: moveresizegame(self,
                                              gobject.baseobject.textsource.hwnd) if gobject.baseobject.textsource.hwnd else 0),
            ...
            ("keepontop", lambda: globalconfig.__setitem__("keepontop", not globalconfig[
                'keepontop']) is None and self.refreshtoolicon() is None and self.setontopthread()),
        ...
        for btn, func in functions:
            belong = globalconfig['toolbutton']['buttons'][btn]['belong'] if 'belong' in \
                                                                             globalconfig['toolbutton']['buttons'][
                                                                                 btn] else None
    ...
    def showEvent(self, a0) -> None:
        if self.isfirstshow:
            ...
            settingAction = QAction(_TR("&设置"), self,
    ...
    def canceltop(self, hwnd=windows.HWND_NOTOPMOST):
        windows.SetWindowPos(int(int(self.winId())), hwnd, 0, 0, 0, 0,
                             windows.SWP_NOACTIVATE | windows.SWP_NOSIZE | windows.SWP_NOMOVE)

    def settop(self):
        windows.SetWindowPos(int(int(self.winId())), windows.HWND_TOPMOST, 0, 0, 0, 0,
                             windows.SWP_NOACTIVATE | windows.SWP_NOSIZE | windows.SWP_NOMOVE)
    ...
    def __init__(self):
        ...
        super(QUnFrameWindow, self).__init__(
            None, flags=Qt.FramelessWindowHint | Qt.WindowMinimizeButtonHint, dic=globalconfig,
            key='transuigeo')  # 设置为顶级窗口，无边框
    ...
    def set_color_transparency(self):
        ...
        self._TitleLabel.setStyleSheet("border-width: 0;\
                                           ...
                                           background-color: rgba(%s, %s, %s, %s)"
                                       % (
                                       int(globalconfig['backcolor'][1:3], 16), int(globalconfig['backcolor'][3:5], 16),
                                       int(globalconfig['backcolor'][5:7], 16), globalconfig['transparent'] / 200))
    ...
    def changemousetransparentstate(self, idx):
        ...
        def _checkplace():
            ...
            while self.mousetransparent:
                ...
                if self.isinrect(cursor_pos, [self._TitleLabel.x(), self._TitleLabel.x() + self._TitleLabel.width(),
                                              self._TitleLabel.y(), self._TitleLabel.y() + self._TitleLabel.height()]):

                    windows.SetWindowLong(int(self.winId()), windows.GWL_EXSTYLE, windows.GetWindowLong(hwnd,
                                                                                                        windows.GWL_EXSTYLE) & ~ windows.WS_EX_TRANSPARENT)
                else:
                    windows.SetWindowLong(int(self.winId()), windows.GWL_EXSTYLE,
                                          windows.GetWindowLong(hwnd, windows.GWL_EXSTYLE) | windows.WS_EX_TRANSPARENT)
                if self.isinrect(cursor_pos, [self._TitleLabel.x(), self._TitleLabel.x() + self._TitleLabel.width(),
                                              self._TitleLabel.y(),
        ...
        for i, button in enumerate(self.buttons[:-2]):
            if button.belong:
                ...
                if hide:
                    button.hide()
                    continue
            if button.name in globalconfig['toolbutton']['buttons'] and \
                    globalconfig['toolbutton']['buttons'][button.name]['use'] == False:
                button.hide()
                continue
