...
def gethiragrid(self):
    ...
    for name in globalconfig['hirasetting']:
        ...
        line += [
            ...
            getsimpleswitch(globalconfig['hirasetting'][name], 'use', parent=self, name=name,
                            callback=functools.partial(yuitsu_switch, self, globalconfig['hirasetting'], 'hiraswitchs',
                                                       name, gobject.baseobject.starthira), pair='hiraswitchs'),

        ]
        items = []
        for key in globalconfig['hirasetting'][name]:
            if key == 'path':
                items.append({'t': 'file', 'l': globalconfig['hirasetting'][name]['name'],
                              'd': globalconfig['hirasetting'][name], 'k': 'path', 'dir': True})
            elif key == 'token':
                items.append({'t': 'lineedit', 'l': globalconfig['hirasetting'][name]['token_name'],
                              'd': globalconfig['hirasetting'][name], 'k': 'token'})
            elif key == 'codec':
                items.append({'t': 'combo', 'l': 'codec', 'd': globalconfig['hirasetting'][name], 'k': 'codec',
                              'list': ['utf8', 'shiftjis']})
        if len(items):
            items.append({'t': 'okcancel', 'callback': gobject.baseobject.starthira})
            line += [getcolorbutton(globalconfig, '', callback=functools.partial(autoinitdialog, self,
...
def setTabcishu_l(self):
    grids = [
                [('分词&假名分析器', 10)],
                [('日语注音方案', 5),
                 (getsimplecombobox(_TRL(['平假名', '片假名', '罗马音']), globalconfig, 'hira_vis_type'), 5)],
            ] + gethiragrid(self) + [
                [],
                [],
                [('点击单词查词', 5), (getsimpleswitch(globalconfig, 'usesearchword'), 1),
                 getcolorbutton(globalconfig, '', callback=lambda: gobject.baseobject.searchwordW.showsignal.emit(),
                                icon='fa.search', constcolor="#FF69B4"), '',
