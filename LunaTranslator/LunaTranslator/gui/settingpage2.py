...
def initsome11(self, l, label=None):
    ...
    for fanyi in globalconfig['fanyi']:
        ...
        if fanyi in translatorsetting:
            ...
            last = getcolorbutton(globalconfig, '', callback=functools.partial(autoinitdialog, self,
                                                                               (globalconfig['fanyi'][fanyi]['name']),
                                                                               800, items), icon='fa.gear',
                                  constcolor="#FF69B4")
        elif fanyi == 'selfbuild':
            last = getcolorbutton(globalconfig, '', callback=lambda: selectdebugfile('./userconfig/selfbuild.py'),
                                  icon='fa.gear', constcolor="#FF69B4")
        else:
            last = ''
        line += [(globalconfig['fanyi'][fanyi]['name'], 6),
                 getsimpleswitch(globalconfig['fanyi'][fanyi], 'use',
                                 callback=functools.partial(gobject.baseobject.prepare, fanyi)),
                 getcolorbutton(globalconfig['fanyi'][fanyi], 'color', parent=self, name="fanyicolor_" + fanyi,
                                callback=functools.partial(selectcolor, self, globalconfig['fanyi'][fanyi], 'color',
                                                           None, self, "fanyicolor_" + fanyi)), last]
        ...
        while True:
            ...
            try:
                ...
                if (checkportavailable(port)):
                    ...
                    if needstart:
                        call = "\"%s\" --disable-extensions --remote-allow-origins=* --disable-gpu --no-first-run --remote-debugging-port=%d --user-data-dir=\"%s\"" % (
                        _path, port, os.path.abspath('./chrome_cache/' + hashtext(_path)))
                        print(call)
...
def setTabTwo_lazy(self):
    ...
    developgrid = [
        [('Chromium_路径', 8), (getcolorbutton(globalconfig, '',
                                               callback=functools.partial(autoinitdialog, self, 'Chromium_路径', 800,
                                                                          _items), icon='fa.gear',
                                               constcolor="#FF69B4"))],
