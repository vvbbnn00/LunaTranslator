...
def setTabThree_direct(self):
    ...
    self.show_original_switch = getsimpleswitch(globalconfig, 'isshowrawtext',
...
class dialog_toolbutton(QDialog):
    ...
    def newline(self, row, k):
        ...
        self.model.insertRow(row, [QStandardItem(), QStandardItem(), QStandardItem(),
                                   QStandardItem(_TR(globalconfig['toolbutton']['buttons'][k]['tip']) + " " + belong)])
        self.table.setIndexWidget(self.model.index(row, 0),
                                  getsimpleswitch(globalconfig['toolbutton']['buttons'][k], 'use', callback=lambda
                                      _: gobject.baseobject.translation_ui.showhidetoolbuttons()))

        self.table.setIndexWidget(self.model.index(row, 1), getcolorbutton('', '', lambda: dialog_selecticon(self,
                                                                                                             globalconfig[
                                                                                                                 'toolbutton'][
                                                                                                                 'buttons'][
                                                                                                                 k],
        ...
        if 'icon2' in globalconfig['toolbutton']['buttons'][k]:
            self.table.setIndexWidget(self.model.index(row, 2), getcolorbutton('', '', lambda: dialog_selecticon(self,
                                                                                                                 globalconfig[
                                                                                                                     'toolbutton'][
                                                                                                                     'buttons'][
                                                                                                                     k],
...
class dialog_selecticon(QDialog):
    def __init__(self, parent, dict, key) -> None:
        ...
        for i, name in enumerate(js):
            layout.addWidget(getcolorbutton('', '', functools.partial(self.selectcallback, 'fa.' + name),
                                            qicon=qtawesome.icon('fa.' + name, color=globalconfig['buttoncolor'])),
                             i // 30, i % 30)
        self.show()
...
def setTabThree_lazy(self):
    ...
    textgrid = [
        ...
        [('字体大小', 3), (self.fontSize_spinBox, 2), '', ('额外的行间距', 3),
         (getspinbox(0, 100, globalconfig, 'extra_space'), 2)],
        [('居中显示', 4), getsimpleswitch(globalconfig, 'showatcenter'), '', ('加粗字体', 4),
         getsimpleswitch(globalconfig, 'showbold')],
        ['', ],
        [('字体样式', 3), (
        getsimplecombobox(_TRL(['普通字体', '空心字体', '描边字体', '阴影字体', '描边字体_reverse']), globalconfig,
                          'zitiyangshi'), 5)],
        [('特殊字体样式填充颜色', 4), getcolorbutton(globalconfig, 'miaobiancolor', transparent=False,
                                                     callback=lambda: selectcolor(self, globalconfig, "miaobiancolor",
                                                                                  self.miaobian_color_button),
                                                     name='miaobian_color_button', parent=self)],
        [('空心线宽', 3), (getspinbox(0.1, 100, globalconfig, 'miaobianwidth', double=True, step=0.1), 2), '',
         ('描边宽度', 3), (getspinbox(0.1, 100, globalconfig, 'miaobianwidth2', double=True, step=0.1), 2), '',
         ('阴影强度', 3), (getspinbox(1, 20, globalconfig, 'shadowforce'), 2)],
        [],

        [('显示原文', 4), self.show_original_switch, '', ('原文颜色', 4), getcolorbutton(globalconfig, 'rawtextcolor',
                                                                                         ...
                                                                                         parent=self)],
        [('显示日语注音', 4), self.show_hira_switch, '', ('注音颜色', 4), getcolorbutton(globalconfig, 'jiamingcolor',
                                                                                         callback=lambda: selectcolor(
                                                                                             self, globalconfig,
                                                                                             "jiamingcolor",
                                                                                             self.jiamingcolor_b),
                                                                                         name='jiamingcolor_b',
                                                                                         parent=self), '',
         ('注音字体缩放', 3), (getspinbox(0.05, 1, globalconfig, 'kanarate', double=True, step=0.05, dec=2), 2)],
        [('语法加亮', 4), self.show_fenciswitch, '',
         ('词性颜色(需要Mecab)', 4),
         getcolorbutton(globalconfig, '', callback=lambda: multicolorset(self), icon='fa.gear',
                        constcolor="#FF69B4"), ],
        [("显示翻译器名称", 4), (getsimpleswitch(globalconfig, 'showfanyisource'), 1), '', ("显示翻译", 4),
        ...
        [],
        [('可选取模式(阴影字体下无效)', 6), getsimpleswitch(globalconfig, 'selectable', callback=lambda
            x: gobject.baseobject.translation_ui.translate_text.setselectable())],
    ]
    ...
    uigrid = [
        ...
        [('翻译窗口背景颜色', 4), getcolorbutton(globalconfig, 'backcolor',
                                                 callback=lambda: selectcolor(self, globalconfig, "backcolor",
                                                                              self.back_color_button,
                                                                              callback=gobject.baseobject.translation_ui.set_color_transparency),
                                                 name='back_color_button', parent=self), '', ('工具按钮颜色', 4),
         getcolorbutton(globalconfig, 'buttoncolor',
                        callback=lambda: selectcolor(self, globalconfig, 'buttoncolor', self.buttoncolorbutton,
                                                     callback=lambda: gobject.baseobject.translation_ui.refreshtooliconsignal.emit()),
                        name='buttoncolorbutton', parent=self), '', ('工具按钮大小', 4), (
         getspinbox(5, 100, globalconfig, 'buttonsize',
                    callback=lambda _: gobject.baseobject.translation_ui.refreshtooliconsignal.emit()), 2)],
        [],
        [('窗口按钮设置', 6),
         getcolorbutton(globalconfig, '', callback=lambda x: dialog_toolbutton(self), icon='fa.gear',
                        constcolor="#FF69B4")],
        [],
        ...
        [("自动隐藏窗口", 6), (getsimpleswitch(globalconfig, 'autodisappear'), 1), '', ("隐藏延迟(s)", 3),
        ...
        [('选择文本窗口中文本框只读', 6), getsimpleswitch(globalconfig, 'textboxreadonly', callback=lambda
            x: gobject.baseobject.hookselectdialog.textOutput.setReadOnly(x))]
    ]
    ...
    innermagpie = [
        ...
        [("常规", 4)],
        [('', 1), ('缩放模式', 4), (getsimplecombobox([_['name'] for _ in magpie10_config['scalingModes']],
                                                      magpie10_config['profiles'][globalconfig['profiles_index']],
                                                      'scalingMode'), 6), ''],
        [('', 1), ('捕获模式', 4), (
        getsimplecombobox(['Graphics Capture', 'Desktop Duplication', 'GDI', 'DwmSharedSurface'],
                          magpie10_config['profiles'][globalconfig['profiles_index']], 'captureMethod'), 6)],
        [('', 1), ('3D游戏模式', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], '3DGameMode'))],
        [("性能", 4)],
        [('', 1), ('显示帧率', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'showFPS'))],
        [('', 1), ('垂直同步', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'VSync'))],
        [('', 1), ('垂直同步_允许额外的延迟以提高性能', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'tripleBuffering'))],
        [("源窗口", 4)],
        [('', 1), ('缩放时禁用窗口大小调整', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'disableWindowResizing'))],
        [('', 1), ('捕获标题栏', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'captureTitleBar'))],
        [('', 1), ('自定义剪裁', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'croppingEnabled'))],
        [("光标", 4)],
        [('', 1), ('绘制光标', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'drawCursor'))],
        [('', 1), ('绘制光标_缩放系数', 4), (
        getsimplecombobox(['0.5x', '0.75x', '无缩放', '1.25x', '1.5x', '2x', '和源窗口相同'],
                          magpie10_config['profiles'][globalconfig['profiles_index']], 'cursorScaling'), 6)],
        [('', 1), ('绘制光标_插值算法', 4), (
        getsimplecombobox(['最邻近', '双线性'], magpie10_config['profiles'][globalconfig['profiles_index']],
                          'cursorInterpolationMode'), 6)],
        [('', 1), ('缩放时调整光标速度', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'adjustCursorSpeed'))],
        [("高级", 4)],
        [('', 1), ('禁用DirectFlip', 4),
         (getsimpleswitch(magpie10_config['profiles'][globalconfig['profiles_index']], 'disableDirectFlip'))],
    ...
    commonfsgrid = [
        [('缩放方式', 4), (
        getsimplecombobox(_TRL(['Magpie', 'ALT+ENTER', 'SW_SHOWMAXIMIZED', 'LosslessScaling', 'Magpie10_External']),
                          globalconfig, 'fullscreenmethod_3'), 6)]
    ]

    losslessgrid = [
        [('Magpie10_路径', 4), (getcolorbutton(globalconfig, '',
                                               callback=lambda x: getsomepath1(self, 'Magpie10_路径', globalconfig,
                                                                               'magpie10path', 'Magpie10_路径',
                                                                               isdir=True), icon='fa.gear',
                                               constcolor="#FF69B4"), 1)],
        [('Hook Magpie进程使其不会退出缩放', 4), getsimpleswitch(globalconfig, 'hookmagpie')],
        [],
        [('LosslessScaling_路径', 4), (getcolorbutton(globalconfig, '',
                                                      callback=lambda x: getsomepath1(self, 'LosslessScaling_路径',
                                                                                      globalconfig['lossless'], 'path',
                                                                                      'LosslessScaling_路径',
                                                                                      isdir=True), icon='fa.gear',
                                                      constcolor="#FF69B4"), 1)],
