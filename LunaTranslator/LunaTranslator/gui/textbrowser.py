                    ...
                    if word['orig'] not in ['\n', '\r', ' ', '']:
                        ...
                        if tl1.y() != tl3.y():
                            ...
                            if globalconfig['usesearchword'] or globalconfig['usecopyword']:
                                ...
                                self.searchmasklabels_clicked[labeli].company = self.searchmasklabels_clicked[
                                    labeli + 1]
                                if callback:
                                    self.searchmasklabels_clicked[labeli].callback = functools.partial(callback,
                                                                                                       (word['orig']))

                                self.searchmasklabels_clicked[labeli + 1].setGeometry(*pos2)
                                self.searchmasklabels_clicked[labeli + 1].show()
                                self.searchmasklabels_clicked[labeli + 1].company = self.searchmasklabels_clicked[
                                    labeli]
                                if callback:
                                    self.searchmasklabels_clicked[labeli + 1].callback = functools.partial(callback, (
                                    word['orig']))
            ...
            while True:
                ...
                for word in line:
                    if word['hira'] == word['orig'] or word['hira'] == "" or word['orig'] == "":
                        ...
                        if len(newline) > 0 and canmerge and (
                                word['hira_w'] + newline[-1]['hira_w'] > word['orig_w'] + newline[-1]['orig_w']):
    ...
    def solvejiaminglabel(self, labels, labeli, word, font, tl1, tl2, fh):
        ...
        if globalconfig['zitiyangshi'] == 3:
            label.setGraphicsEffect(
                self.geteffect(globalconfig['fontsize'], globalconfig['jiamingcolor'], globalconfig['shadowforce']))
