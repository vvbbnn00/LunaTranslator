...
class Settin(closeashidewindow):
        ...
        for grid, maxl in self.needfitcols:
            for c in range(maxl):
                grid.setColumnMinimumWidth(c,
                                           int(self.size().width() - self.window_width * 0.2 - self.scrollwidth // maxl))
        ...
        if self.isfirstshow:
            ...
            self.tab_widget.setStyleSheet(
                '''QTabBar#basetabbar:tab { 
                    ...
                    height: %spx;
                    font:%spt  ;  }
                ''' % (50, self.window_width * 0.2,
                       globalconfig['tabfont_chs'] if globalconfig['languageuse'] == 0 else globalconfig[
                           'tabfont_otherlang'])
            )
    ...
    def setstylesheet(self):
        self.setStyleSheet(
            "font: %spt '" % (globalconfig['settingfontsize']) + (globalconfig['settingfonttype']) + "' ;  ")
