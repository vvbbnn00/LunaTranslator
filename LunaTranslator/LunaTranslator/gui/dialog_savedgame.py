...
from PyQt5.QtWidgets import QHBoxLayout, QTableView, QAbstractItemView, QLabel, QVBoxLayout, QSpacerItem, QTabWidget, \
    QComboBox
import importlib
import windows
...
class ItemWidget(QWidget):
    ...
    def focusInEvent(self, ev) -> None:
        self.bottommask.setStyleSheet(
            'QLabel { background-color: ' + globalconfig['dialog_savegame_layout']['onselectcolor'] + '; }')
    ...
    def latershowfileexits(self, exe):
        if os.path.exists(exe):
            ...
            self.maskshowfileexists.setStyleSheet(
                'QLabel { background-color: ' + globalconfig['dialog_savegame_layout']['onfilenoexistscolor'] + '; }')
...
class browserdialog(QDialog):
    ...
    def resizeEvent(self, a0: QResizeEvent) -> None:
        ...
        rate = getScreenRate()
        rect = 0, int(rate * self.nettab.height()), int(rate * a0.size().width()), int(
            rate * (a0.size().height() - self.nettab.height()))
        if self.webviewv == 0:
    ...
    def __init__(self, parent, textsource_or_exepath) -> None:
        ...
        self.hasvndb = bool(
            savehook_new_data[self.exepath]['infopath'] and os.path.exists(savehook_new_data[self.exepath]['infopath']))
        if self.hasvndb:
        ...
        if action == shanchu:
            ...
            def cachehtml(exepath, idx):
                ...
                with open(fn, 'w', encoding='utf8') as ff:
                    ff.write(response.text.replace('<HEAD>', '<HEAD><base href="{}">'.format(url)).replace('<head>',
                                                                                                           '<head><base href="{}">'.format(
                                                                                                               url)))
    ...
    def lastclicked(self):
        ...
        gobject.baseobject.Prompt.call.emit(_TR('添加关联页面'), _TR('页面类型_页面链接'),
                                            [['vndb/2df/...'], 'about:blank'], [callback])
...
class dialog_setting_game(QDialog):
    def selectexe(self):
        ...
        if res != '':
            ...
            if self.item:
                ...
                self.table.setIndexWidget(self.model.index(self.model.indexFromItem(self.item).row(), 1),
                                          getcolorbutton('', '', functools.partial(opendir, res), qicon=_icon))
    ...
    def __init__(self, parent, exepath, item=None, type=1, gametitleitme=None) -> None:
        ...
        if type == 2:
            ...
            statiswids = [
                QLabel(_TR("统计信息")),
                getcolorbutton('', '', lambda: dialog_statistic(self, exepath), icon='fa.bar-chart',
                               constcolor="#FF69B4"),
                QLabel(_TR("vndbid")), vndbid,
                getcolorbutton('', '', lambda: browserdialog(self, exepath), icon='fa.book', constcolor="#FF69B4")
            ]
        ...
        formLayout.addLayout(getboxlayout([
            QLabel(_TR("自动切换到模式")),
            getsimplecombobox(_TRL(['不切换', 'HOOK', '剪贴板', 'OCR', 'FridaHook']), savehook_new_data[exepath],
    ...
    def gethooktab(self, exepath):
        ...
        formLayout.addLayout(getboxlayout([
            QLabel(_TR('代码页')),
            getsimplecombobox(_TRL(static_data['codepage_display']), savehook_new_data[exepath], 'codepage_index',
        ...
        if savehook_new_data[exepath]['use_saved_text_process'] or 'save_text_process_info' in savehook_new_data[
            exepath]:
            formLayout.addLayout(getboxlayout([
    ...
    def newline(self, row, k):
        ...
        self.hctable.setIndexWidget(self.hcmodel.index(row, 0),
                                    getcolorbutton('', '', self.clicked2, icon='fa.times', constcolor="#FF69B4"))
...
class dialog_syssetting(QDialog):
    ...
    def __init__(self, parent) -> None:
        ...
        for key, name in [('onselectcolor', '选中时颜色'), ('onfilenoexistscolor', '游戏不存在时颜色')]:
            formLayout.addRow(QLabel(_TR(name)), getcolorbutton(globalconfig['dialog_savegame_layout'], key,
                                                                callback=functools.partial(selectcolor, self,
                                                                                           globalconfig[
        ...
        if os.path.exists(game):
            ...
            if mode == 0:
                ...
                if globalconfig['sourcestatus2'][_[mode]]['use'] == False:
                    ...
                    yuitsu_switch(gobject.baseobject.settin_ui, globalconfig['sourcestatus2'], 'sourceswitchs', _[mode],
                                  None, True)
                    gobject.baseobject.starttextsource(use=_[mode], checked=True)
            if globalconfig['sourcestatus2']['fridahook']['use'] and savehook_new_data[game]['fridahook'].get(
                    'loadmethod') == 1:
            ...
            windows.CreateProcess(None, '"{}" {} {}'.format(shareddllproxy, _cmd, usearg), None, None, False, 0, None,
                                  dirpath, windows.STARTUPINFO())
    except:
        print_exc()
...
class dialog_savedgame_new(saveposwindow):
    ...
    def __init__(self, parent) -> None:
        super().__init__(parent, flags=Qt.WindowMinMaxButtonsHint | Qt.WindowCloseButtonHint, dic=globalconfig,
    ...
    def showsettingdialog(self):
        ...
        dialog_setting_game(self, self.currentfocuspath, None, type=2,
...
class dialog_savedgame(QDialog):
    ...
    def newline(self, row, k):
        ...
        k = k.replace('/', '\\')
        checkifnewgame(k)
        self.model.insertRow(row, [QStandardItem(), QStandardItem(), keyitem,
        ...
        self.table.setIndexWidget(self.model.index(row, 1), getcolorbutton('', '', functools.partial(opendir, k),
                                                                           qicon=getExeIcon(k, cache=True)))

        self.table.setIndexWidget(self.model.index(row, 2),
                                  getcolorbutton('', '', functools.partial(self.showsettingdialog, k, keyitem),
                                                 icon='fa.gear', constcolor="#FF69B4"))
