...
def setTab_quick_direct(self):
    ...
    self.bindfunctions = {
        ...
        "_25": lambda: windows.SendMessage(windows.FindWindow('Magpie_Core_CLI_Message', None),
...
def setTab_quick_lazy(self):
    grids = [
        [(("是否使用快捷键"), 4),
         getsimpleswitch(globalconfig['quick_setting'], 'use', callback=functools.partial(__enable, self)), ((''), 8)]
    ]
    for name in globalconfig['quick_setting']['all']:
        if name not in self.bindfunctions:
            continue
        ...
        grids.append(
            [((globalconfig['quick_setting']['all'][name]['name']), 4),
             getsimpleswitch(globalconfig['quick_setting']['all'][name], 'use',
...
def regist_or_not_key(self, name):
    ...
    if keystring == '' or (
    not (globalconfig['quick_setting']['all'][name]['use'] and globalconfig['quick_setting']['use'])):
        return
