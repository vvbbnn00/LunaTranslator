...
def getversion(self):
    self.versiontextsignal.emit(
        ('当前版本') + ':' + static_data["version"] + '  ' + ("最新版本") + ':' + ('获取中'))  # ,'',url,url))
    ...
    if _version is None:
        ...
        sversion = _version
    self.versiontextsignal.emit(('{}:{}  {}  {}:{}'.format(_TR("当前版本"), static_data["version"],
                                                           platform.architecture()[0], _TR("最新版本"),
                                                           sversion)))  # ,'' if static_data["version"]== _version else  newcontent,url,'LunaTranslator.zip'))
...
def setTab_aboutlazy(self):
    grid2 = [
        [('自动下载更新(需要连接github)', 5),
         (getsimpleswitch(globalconfig, 'autoupdate', callback=lambda x: getversion(self)), 1), ('', 10)],
