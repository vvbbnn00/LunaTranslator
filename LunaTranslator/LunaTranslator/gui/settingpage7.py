...
from PyQt5.QtWidgets import QDialog, QLabel, QLineEdit, QSpinBox, QPushButton, QTableView, QVBoxLayout, QHBoxLayout, \
    QHeaderView, QTextEdit, QHBoxLayout, QWidget, QMenu, QAction
    ...
    try:
        try:
            with open('./userconfig/mypost.py', 'r', encoding='utf8') as ff:
                _mypost = ff.read()
            with open('./userconfig/posts/{}.py'.format(gobject.baseobject.textsource.uuname), 'w',
                      encoding='utf8') as ff:
                ff.write(_mypost)
        except:
            _mypost = None
...
def settab7direct(self):
    ...
    self.button_noundict = getcolorbutton(globalconfig, '',
                                          callback=lambda x: noundictconfigdialog(self, noundictconfig,
                                                                                  '专有名词翻译设置(游戏ID 0表示全局)'),
                                          icon='fa.gear', constcolor="#FF69B4")
    self.button_fix = getcolorbutton(globalconfig, '',
                                     callback=lambda x: noundictconfigdialog1(self, transerrorfixdictconfig,
                                                                              '翻译结果替换设置', ['翻译', '替换'],
                                                                              './userconfig/transerrorfixdictconfig.json'),
                                     icon='fa.gear', constcolor="#FF69B4")
...
def getcomparelayout(self):
    ...
    fromtext = QTextEdit()
    totext = QTextEdit()
    solvebutton = getcolorbutton(globalconfig, '',
                                 callback=lambda: totext.setPlainText(POSTSOLVE(fromtext.toPlainText())),
                                 icon='fa.chevron-right', constcolor="#FF69B4")
...
def setTab7_lazy(self):
    ...
    def changerank(item, up):
        ...
        savelist[ii + headoffset], savelist[toexchangei + headoffset] = savelist[toexchangei + headoffset], savelist[
            ii + headoffset]

    for i, post in enumerate(sortlist):
        if post == '_11':
            config = (getcolorbutton(globalconfig, '', callback=lambda: selectdebugfile('./userconfig/mypost.py'),
                                     icon='fa.gear', constcolor="#FF69B4"))
        else:
            if post not in postprocessconfig:
                continue
            if post == '_remove_chaos':
                config = (getcolorbutton(globalconfig, '', icon='fa.gear', constcolor="#FF69B4",
                ...
                if isinstance(list(postprocessconfig[post]['args'].values())[0], dict):
                    callback = functools.partial(postconfigdialog, self, postprocessconfig[post]['args'],
        ...
        button_up = (
            getcolorbutton(globalconfig, '', callback=functools.partial(changerank, post, True), icon='fa.arrow-up',
                           constcolor="#FF69B4"))
        button_down = (
            getcolorbutton(globalconfig, '', callback=functools.partial(changerank, post, False), icon='fa.arrow-down',
                           constcolor="#FF69B4"))
    ...
    grids2 = [
        ...
        [(('使用VNR共享辞书'), 6),
         ...
         getcolorbutton(globalconfig, '',
                        callback=lambda x: getsomepath1(self, '共享辞书', globalconfig['gongxiangcishu'], 'path',
                                                        '共享辞书', gobject.baseobject.loadvnrshareddict, False,
                                                        '*.xml'), icon='fa.gear', constcolor="#FF69B4"), '', '', '', '',
         '', ''],
    ...
    def closeEvent(self, a0: QCloseEvent) -> None:
        ...
        for row in range(rows):
            ...
            if self.model.item(row, 1).text() not in newdict:
                newdict[self.model.item(row, 1).text()] = [self.model.item(row, 0).text(),
