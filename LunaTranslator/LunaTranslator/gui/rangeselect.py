...
class rangeadjust(Mainw):
    ...
    def setstyle(self):
        self.label.setStyleSheet(" border:%spx solid %s; background-color: rgba(0,0,0, 0.01)" % (
    ...
    def moveEvent(self, e):
        rect = self.geometry()
        if self._rect:
            self._rect = [(rect.left() + globalconfig['ocrrangewidth'], rect.top() + globalconfig['ocrrangewidth']),
                          (rect.right() - globalconfig['ocrrangewidth'], rect.bottom() - globalconfig['ocrrangewidth'])]
    ...
    def setrect(self, rect):
        ...
        if rect:
            (x1, y1), (x2, y2) = rect
            self.setGeometry(x1 - globalconfig['ocrrangewidth'], y1 - globalconfig['ocrrangewidth'],
                             x2 - x1 + 2 * globalconfig['ocrrangewidth'], y2 - y1 + 2 * globalconfig['ocrrangewidth'])
            self.show()
...
class rangeselct(QMainWindow):
    ...
    def reset(self):
        ...
        self.rectlabel.setStyleSheet(" border:%spx solid %s; background-color: rgba(0,0,0, 0.01)" % (
        globalconfig['ocrrangewidth'], globalconfig['ocrrangecolor']))
...
class moveresizegame(QDialog):
    ...
    def moveEvent(self, a0) -> None:
        ...
        if self.isMaximized() == False:
            try:
                windows.MoveWindow(self.hwnd, rect.left(), rect.top(), rect.right() - rect.left(),
