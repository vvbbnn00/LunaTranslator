...
def _7_zhuanyi_f(line, args):
    ...
    for fil in filters:
        if fil == "":
            ...
            line = line.replace(codecs.escape_decode(bytes(fil, "utf-8"))[0].decode("utf-8"),
...
def _8_f(line, args):
    ...
    for fil in filters:
        if fil == "":
            ...
            try:
                line = re.sub(codecs.escape_decode(bytes(fil, "utf-8"))[0].decode("utf-8"),
                              codecs.escape_decode(bytes(filters[fil], "utf-8"))[0].decode("utf-8"), line)
            except:
                print_exc()
    return line
