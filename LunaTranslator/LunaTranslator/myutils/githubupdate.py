...
def getvesionmethod():
    ...
    try:
        ...
        res = requests.get('https://api.github.com/repos/HIllya51/LunaTranslator/releases/latest', headers=headers,
                           verify=False, proxies=getproxy()).json()
        # print(res)
    ...
    def checkalready(size):
        if os.path.exists(savep):
            ...
            if stats.st_size == size:
                progresscallback('总大小{} MB 进度 {}% '.format(int(1000 * (int(size / 1024) / 1024)) / 1000,
                                                                int(10000 * (size / size)) / 100), 10000)
                endcallback()
                return True
        return False
