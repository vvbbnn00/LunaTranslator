...
from traceback import print_exc
from myutils.config import globalconfig, static_data, savehook_new_list, savehook_new_data, getdefaultsavehook, \
    translatorsetting
...
def checkinfo(gamepath):
    return (savehook_new_data[gamepath]['infopath'] is None) or (
                (savehook_new_data[gamepath]['infopath'][:4].lower() != 'http') and os.path.exists(
            savehook_new_data[gamepath]['infopath']) == False)
...
def dispatachtask(gamepath):
    ...
    if savehook_new_data[gamepath]['vid']:
        ...
        for _ in [savehook_new_data[gamepath]['title'], os.path.basename(os.path.dirname(gamepath)),
        ...
        try:
            ...
            if event == windows.EVENT_SYSTEM_MINIMIZEEND:
                ...
                if globalconfig['movefollow']:
                    if self.lastpos:
                        rate = getScreenRate()
                        self.hookfollowsignal.emit(5, (
                        int((rect[0] - self.lastpos[0]) / rate), int((rect[1] - self.lastpos[1]) / rate)))
