...
class fullscreen():
    ...
    def runmagpie10(self):
        if windows.FindWindow('Magpie_Hotkey', None) == 0:
            subproc_w(os.path.join(globalconfig['magpie10path'], 'Magpie.exe'), cwd=globalconfig['magpie10path'],
                      name='magpie10')
    ...
    def _magpie_builtin(self, hwnd, full):
        if full:
            ...
            self.engine = subproc_w(
                './files/plugins/Magpie10/Magpie.Core.exe {} {} "{}"'.format(profiles_index, hwnd, jspath),
                cwd='./files/plugins/Magpie10/')
            self._waitenginestop_magpie()
        else:
            windows.SendMessage(windows.FindWindow('Magpie_Core_CLI_Message', None),
