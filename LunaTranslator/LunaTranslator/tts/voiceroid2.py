    ...
    def checkpath(self):
        ...
        if self.config["path"] != self.path or self.config["voice"] != self.voice or globalconfig["ttscommon"][
            ...
            self.engine = autoproc(subproc_w(
                '"{}" voiceroid2 "{}" "{}" {} 44100 {} "{}"  {} {}'.format(exepath, self.config["path"], dllpath,
                                                                           self.config["voice"], linear_map(
                        globalconfig["ttscommon"]["rate"]), savepath, pipename, waitsignal), name='voicevoid2'))
