    ...
    def __init__(self):
        ...
        try:
            if os.path.exists(os.path.join(globalconfig['cishu']['linggesi']['path'], 'ja-zh.db')) == False or \
                ...
                return
            self.sql = autosql(sqlite3.connect(os.path.join(globalconfig['cishu']['linggesi']['path'], 'ja-zh.db'),
                                               check_same_thread=False))
            self.sql2 = (sqlite3.connect(os.path.join(globalconfig['cishu']['linggesi']['path'], 'ja-zh-gbk.db'),
                                         check_same_thread=False))
        except:
            pass
