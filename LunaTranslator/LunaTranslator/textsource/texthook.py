...
from ctypes import CDLL, c_bool, POINTER, Structure, c_int, pointer, c_wchar_p, c_uint64, sizeof, c_void_p, cast, \
    c_wchar, c_uint32, c_uint8, c_uint, c_char, c_short
...
class texthook(basetext):
    ...
    def flashembedsettings(self, pid=None):
        ...
        for pid in pids:
            self.Luna_EmbedSettings(pid,
                                    ...
                                    globalconfig['embedded']['changefont_font'] if globalconfig['embedded'][
                                        'changefont'] else '',
    ...
    def match_compatibility(self, key, autostarthookcode):
        ...
        autostarthookcode[2] & 0xffff, autostarthookcode[3] & 0xffff, autostarthookcode[5])
        name = ((key[-1][:8] == 'UserHook' and autostarthookcode[-1][:8] == 'UserHook') or (
                    key[-1] == autostarthookcode[-1]))
    ...
    def setsettings(self):
        self.Luna_Settings(globalconfig['textthreaddelay'], globalconfig['direct_filterrepeat'], self.codepage(),
