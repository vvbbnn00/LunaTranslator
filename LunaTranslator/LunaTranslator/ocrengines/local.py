...
class OCR(baseocr):
    ...
    def checkchange(self):
        ...
        failed = False
        if not (os.path.exists(path + '/det.onnx') and os.path.exists(path + '/rec.onnx') and os.path.exists(
                path + '/dict.txt')):
            failed = True
            if (innerlang == 'en'):
                path2 = './files/ocr/{}'.format('ja')
                if (os.path.exists(path2 + '/det.onnx') and os.path.exists(path2 + '/rec.onnx') and os.path.exists(
                        path2 + '/dict.txt')):
                    path = path2
                    failed = False
    ...
    def ocr(self, imgfile):
        ...
        if self.isusingjaasen:
            res = '<msg_warning_with_text>' + _TR(
                '未下载该语言的OCR模型,请从软件主页下载模型解压到files/ocr路径后使用') + '\n' + _TR(
                '正在使用日语模型作为代替，可能效果不佳') + '</msg_warning_with_text>' + res
        return res
